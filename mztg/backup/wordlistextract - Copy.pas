...
//      pagina: IHTMLElement;
    begin
      ...
      for n := 1 to 101 do begin
        Result := StringReplace(Result, Entities[n][2], Entities[n][1],  [rfReplaceAll]);
        Result := StringReplace(Result, Entities[n][3], Entities[n][1],  [rfReplaceAll]);
      end;
...
//                showmessage(StripHTMLTags(lst.Text));
                lst.Text := StripHTMLTags(lst.Text);
                lst.SaveToFile(Arquivo+'.txt');
